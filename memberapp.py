...
form_class = uic.loadUiType("ui/memberUI.ui")[0]  # 미리 제작해 놓은 ui 불러오기
...
class MainWindow(QMainWindow, form_class):
    def __init__(self):
        ...
        self.idcheck_btn.clicked.connect(self.idcheck)  # 회원가입여부 체크 버튼이 클릭되면 가입여부 확인 함수호출
        ...
        self.login_btn.clicked.connect(self.member_login)  # 로그인 버튼 클릭시 회원가입 된
    ...
    def member_join(self):  # 회원 가입 이벤트 처리 함수
        ...
        if memberid == "" or memberpw == "" or membername == "" or memberemail == "" or memberage == "":
            ...
            # 위에 회원가입불가 멘트를 사용할 경우 동일한 멘트팝업창이 2번 표시됨
        else:
            ...
            if result == 1:
                QMessageBox.warning(self, "회원기입성공", "축하합니다.\n회원가입이 성공하셨습니다.")
    ...
    def idcheck(self):  # 기존 아이디 회원가입여부 체크 함수
        ...
        # SQL문 실행 시 1 또는 0이 반환(기존에 가입된 아이디면 1, 아니면0
            ...
            if result[0][0] == 1:
                ...
                QMessageBox.warning(self, "회원조회불가", "이미 가입된 아이디입니다. \n다시 입력해주세요.")
                return 0
    ...
    def member_login(self):
        ...
        if loginid == "" or loginpw == "":  # 아이디 또는 비밀번호가 공란인지 확인ㄴ
            ...
            # 아이디와 비밀번호가 모두 일치하는 레코드의 갯수를 반환(1, 0 이 반환) - 둘 다 만족해야 1, 둘 중 하나만 만족하면 0
